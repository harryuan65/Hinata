...
U16 DoMove(U16 move, fighter *board, int *chessboard, int turn) {
    ...
    if (eat) {
        if (pro) {
            *(&board->w_pawn + pce_bias[src_pce]) ^= srcbit;
            src_pce ^= PROMOTE;
            *(&board->w_pawn + pce_bias[src_pce]) ^= dstbit;
        }
        else
            *(&board->w_pawn + pce_bias[src_pce]) ^= srcbit | dstbit;
        *(&board->w_pawn + pce_bias[dst_pce]) ^= dstbit;

        if (turn == WHITE) {
            board->w_occupied ^= srcbit | dstbit;
            board->b_occupied ^= dstbit;
        }
        else {
            board->b_occupied ^= srcbit | dstbit;
            board->w_occupied ^= dstbit;
        }

        return SaveCapture(dst_pce, board, chessboard) << 5 | dst_pce;
    }
    else {
        if (src < BOARD_SIZE) {
            if (pro) {
...
void UnDoMove(U16 capture, U16 move, fighter *board, int *chessboard, int turn) {
    ...
    if (capture == TYPE_NORMAL_MOVE) {
        *(&board->w_pawn + pce_bias[dst_pce]) ^= srcbit | dstbit;
        if (pro) {
            *(&board->w_pawn + pce_bias[dst_pce]) ^= srcbit;
            dst_pce ^= PROMOTE;
            *(&board->w_pawn + pce_bias[dst_pce]) ^= srcbit;
        }
        ...
        if (capture == TYPE_HAND_MOVE) {
            ...
            *(&board->w_pawn + pce_bias[cap_pce]) ^= dstbit;
